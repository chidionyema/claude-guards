...
from estate_broadcast import mirror_to_github, format_row, DEADLETTER as ORIGINAL_DEADLETTER_PATH

# Define test-specific values for GH_REPO and GH_BOARD_ISSUE
TEST_GH_REPO = "chidionyema/crew"
TEST_GH_BOARD_ISSUE = "102"
...
def set_gh_env_vars(monkeypatch):
    """Fixture to set GH_BOARD_REPO and GH_BOARD_ISSUE environment variables for tests."""
    monkeypatch.setenv("GH_BOARD_REPO", TEST_GH_REPO)
    monkeypatch.setenv("GH_BOARD_ISSUE", TEST_GH_BOARD_ISSUE)
...
def test_mirror_to_github_deadletters_on_failure(temp_deadletter_file, mock_subprocess_run, capsys):
    ...
    mock_subprocess_run.assert_called_once_with(
        ["gh", "issue", "comment", TEST_GH_BOARD_ISSUE, "--repo", TEST_GH_REPO, "--body", expected_body],
