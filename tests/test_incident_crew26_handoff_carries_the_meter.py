...
def test_incident_crew26_every_handoff_carries_the_meter(tmp_path, monkeypatch):
    ...
    assert (
        fg.append(
            ...
            "lane",
            BODY,
            meter="📍 METER: 2026-08-27 $1.00 1 req $1.000/req",
        )
        is None
    )
    text = feed.read_text()
...
def test_incident_crew26_meter_is_blind_not_a_guess_when_the_script_is_missing(
    monkeypatch,
):
    import sys
    ...
    monkeypatch.setattr(
        feed_meter, "Path", type("P", (), {})
    )  # any exception path -> BLIND
