...
CEILING = (
    15 * 1024
)  # 14,735 on 2026-08-27; lower this when the file shrinks, never raise it
...
def test_resident_copy_fits_and_moved_blocks_survive_verbatim():
    ...
    # 2026-09-14 ratchet (founder: the table alone was ~32KB of ~37KB, loaded every turn for no
    # reason): the table, the hard rules detail, and the compact-instructions detail moved out of
    # the resident copy too, leaving only the headline and one-line pointers behind.
    for head in (
        "# THE HEADLINE",
        "# Reply format",
    ...
    for moved in (
        "# LAW OF CONTINUOUS EXECUTION",
        "# LAW OF LAZY CONSENSUS",
        "# LAW OF TELEMETRY COVERAGE",
...
def test_founder_blocker_reads_the_register_where_it_now_lives():
    ...
    spec = importlib.util.spec_from_file_location(
        "fb", LAWS.parent / "founder-blocker.py"
    )
    fb = importlib.util.module_from_spec(spec)
    sys.modules["fb"] = spec.loader and spec.loader.exec_module(fb) or fb
