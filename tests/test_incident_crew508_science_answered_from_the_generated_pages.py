...
def test_incident_crew508_science_prompt_injects_lanes_and_grades(tmp_path):
    ...
    for prompt in (
        "science",
        "how is our general purpse reseach cpapbility",  # founder, 2026-08-27, verbatim
        "science covervs everythig so when i say science i need to see progress across al lanes",
        "also data and nachie learning lane"[:0] or "data science lane",
        "how are the lanes",
    ):
        out = m.science_answer(prompt, d, "https://x/science")
        assert (
            out
            and "https://x/science/SHOWCASE.md" in out
            and "| portal | 0 | 0 | BLIND" in out
        ), prompt
        assert (
            "| Outward | **ELITE**" in out
            and "| Inward | **GAP**" in out
            and "| RED 30d" in out
        ), prompt
        assert "| x | y |" not in out and "| Questions asked" not in out, (
            "graded rows only; the page holds the rest"
        )
...
def test_incident_crew508_missing_page_is_blind_with_the_url(tmp_path):
    ...
    assert (
        out
        and out.count("BLIND") == 2
        and "https://x/science/RESEARCH-GRADE.md" in out
        and "never from memory" in out
    )
