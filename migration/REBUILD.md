...
**Windows renders, and 12 of 29 jobs now cross intact.** `jobs/render_windows.py`
turns all 29 jobs into Task Scheduler XML and names, per job, what Windows does
not keep.

The two rows that did not need a Windows machine to fix are fixed. A job used to
name `/usr/bin/python3` and carry a colon-separated `PATH`, which were 36 of the
losses. The program is now a placeholder such as `{PYTHON3_SYSTEM}` and a search
path is now a list of directories; `jobs/platforms.json` is the only file that
holds a platform's own names. That took the reading from 0 of 29 crossing intact
to 12, and from 29 lossy to 17. All 13 POSIX-program losses are gone and the 23
`PATH` losses are down to 2, each naming the specific directory that has no
Windows equivalent rather than condemning the whole variable.
...
What is left is a genuine platform difference in every case: 9 jobs set `Nice`,
6 use `KeepAlive` as a supervisor that restarts on any exit where Task Scheduler
only offers restart-on-failure, 6 set `ThrottleInterval`, 4 set `ProcessType`,
and one uses `WatchPaths`, which Task Scheduler has no trigger for. `--check` is
what counts them, so the number moves when the work lands rather than when
somebody says it did.

**None of this is a proved Windows rebuild.** No rendered task has been imported
on Windows and run. `drills/run.py --list` carries a `windows-rebuild` entry
that says exactly that, so the gap is on the board rather than in somebody's
head.
