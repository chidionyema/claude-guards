...
def test_the_three_boxes_are_build_prove_and_founder_used_it():
    assert tg.has_dod(tg.DOD_BOXES)
    for word in ("Built", "Proved", "Founder used it"):
        assert word in tg.DOD_BOXES


def test_a_body_with_fewer_than_three_boxes_is_not_done_shaped():
    assert not tg.has_dod("- [ ] one\n- [x] two\n")
    assert not tg.has_dod("")
    assert tg.has_dod("- [ ] a\n- [x] b\n- [ ] c\n")


def test_lane_comes_from_the_working_directory_and_never_guesses():
    assert tg.lane_for("/Users/x/dev/code/idp/.wt-abc") == "lane:platform"
    assert tg.lane_for("/Users/x/dev/code/hermes-v2") == "lane:agents"
    assert tg.lane_for("/Users/x/dev/code/prospector-main/app") == "lane:money"
    assert tg.lane_for("/Users/x/dev/code") == "lane:unsorted"
