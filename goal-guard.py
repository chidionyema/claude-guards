...
def _focus_path() -> Path:
    return STATE_DIR / "FOCUS.json"


def read_focus() -> dict:
    """The standing focus, or {} when none is set. Empty text means no focus."""
    try:
        d = json.loads(_focus_path().read_text())
        return d if d.get("text") else {}
    except Exception:
        return {}


def focus(text: str, source: str = "", since_hours: float = FOCUS_SINCE_HOURS) -> list[str]:
    ...
    tmp = _focus_path().with_suffix(".tmp")
    tmp.write_text(json.dumps(rec))
    tmp.replace(_focus_path())
    ledger({"t": int(time.time()), "kind": "focus", "source": source[:120],
...
def focus_filter(items: list, focus_text: str) -> list:
    ...
    words = {w.lower() for w in re.findall(r"[A-Za-z][A-Za-z0-9-]{3,}", focus_text)}
    words -= {"only", "focus", "this", "that", "with", "from", "then", "crew"}
    out = []
    for i in items:
        title = (i.get("title") or "").lower()
        if i.get("number") in nums or any(w in title for w in words):
            out.append(i)
    return out
...
def selftest() -> int:
    ...
    write_state("live1", {"goal": "crew#66: eradicate fly", "run": 0, "last_progress": "", "last_progress_at": 0, "fired": 0, "calls": 0})
    write_state("stale1", {"goal": "crew#13: retire hermes", "run": 0, "last_progress": "", "last_progress_at": 0, "fired": 0, "calls": 0})
    os.utime(state_path("stale1"), (time.time() - 3 * 86400, time.time() - 3 * 86400))
    done = focus("crew#284: finish KINI, nothing else", "selftest")
    ck("the live session is rewritten", read_state("live1")["goal"] == "crew#284: finish KINI, nothing else")
    ck("the old goal is kept as prev_goal", read_state("live1")["prev_goal"] == "crew#66: eradicate fly")
    ck("the stale session is left alone", read_state("stale1")["goal"] == "crew#13: retire hermes")
    ck("the live session is reported and the stale one is not", "live1" in done and "stale1" not in done)
    ck("the focus is on disk", read_focus()["text"].startswith("crew#284"))
    ck("an empty focus rewrites nothing", focus("   ", "selftest") == [])
    items = [{"number": 66, "title": "eradicate fly io"}, {"number": 284, "title": "KINI delivered"},
             {"number": 306, "title": "hard execution chain for kini"}]
    kept = [i["number"] for i in focus_filter(items, read_focus()["text"])]
...
def main() -> int:
    ...
    if "--focus" in sys.argv:
        ...
        src = sys.argv[sys.argv.index("--source") + 1] if "--source" in sys.argv else "cli"
        hrs = float(sys.argv[sys.argv.index("--since-hours") + 1]) if "--since-hours" in sys.argv else FOCUS_SINCE_HOURS
        if not text.strip():
            print("--focus needs the founder's line, e.g. --focus 'crew#284: finish KINI'", file=sys.stderr)
            return 1
        done = focus(text, src, hrs)
