...
def meter_line(
    timeout: float = 5.0,
) -> str:  # crew#786: a handoff never waits on the meter; BLIND after 5 s
    ...
    try:
        ...
        transport = sum(
            float(drv.get(k, 0)) for k in ("cache_read", "cache_write", "raw_input")
        )
        share = (transport / total * 100) if total else 0.0
        models = ", ".join(
            f"{m} {v / total * 100:.0f}%"
            for m, v in sorted(
                (d.get("by_model") or {}).items(), key=lambda kv: -kv[1]
            )[:3]
        )
        return (
            f"{METER_MARK} {d['day']} ${total:,.2f} {reqs:,} req ${per:.3f}/req "
