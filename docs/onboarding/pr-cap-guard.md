...
**What it is.** A PreToolUse fence on `gh pr create`. If the target repo has more than 10 open PRs the command is refused with the count and the three oldest PRs. Ticket: crew#504 (113 open PRs across seven repos on 2026-08-27; the founder: "we have 24 pull requests open this is crazy").
...
**How to change the cap.** `PR_CAP=20` in the session environment; the default is 10, the number in crew#504.
...
**How to know it is working.** `python3 ~/.claude/scripts/pr-cap-guard.py --selftest` prints six PASS lines; `tests/test_incident_crew504_pr_cap_refuses_gh_pr_create.py` drives the script through a fake `gh` at 11 and 10 open.
