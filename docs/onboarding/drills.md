...
`drills/register.json` holds 15 entries. 11 have a command. 4 have no
...
| github-gone | yes | GitHub goes down, suspends the account, or the token is revoked, and every repository's history is unreachable |
...
| substrate-never-sleeps | yes | The MacBook sleeps |
| oke-rebuild | yes | The platform on OKE drifts from git, stops serving, or cannot be rebuilt from its repositories; the estate is not portable and nobody knows until a customer does |
